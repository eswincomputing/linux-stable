...
static const char *es_dvp2axi_pixelcode_to_string(u32 mbus_code)
{
	unsigned int i;
...
static void es_dvp2axi_show_format(struct es_dvp2axi_device *dev, struct seq_file *f)
{
	struct es_dvp2axi_stream *stream = &dev->stream[0];
	struct es_dvp2axi_pipeline *pipe = &dev->pipe;
	...
	if (sensor) {
		...
		seq_printf(f, "\tformat:%s/%ux%u@%d\n",
			   es_dvp2axi_pixelcode_to_string(stream->dvp2axi_fmt_in->mbus_code),
		...
		seq_printf(f, "\tformat:%s/%ux%u(%u,%u)\n",
			   es_dvp2axi_pixelcode_to_string(stream->dvp2axi_fmt_out->fourcc),
...
static int es_dvp2axi_proc_show(struct seq_file *f, void *v)
{
	struct es_dvp2axi_device *dev = f->private;

	if (dev) {
		es_dvp2axi_show_mixed_info(dev, f);
		es_dvp2axi_show_clks(dev, f);
		es_dvp2axi_show_format(dev, f);
	} else {
		seq_puts(f, "dev null\n");
	}
...
static int es_dvp2axi_proc_open(struct inode *inode, struct file *file)
{
	struct es_dvp2axi_device *data = pde_data(inode);

	return single_open(file, es_dvp2axi_proc_show, data);
}

static const struct proc_ops es_dvp2axi_proc_fops = {
	.proc_open = es_dvp2axi_proc_open,

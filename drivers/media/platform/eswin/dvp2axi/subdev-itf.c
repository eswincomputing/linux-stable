...
static void sditf_buffree_work(struct work_struct *work)
{
	...
	while (!list_empty(&local_list)) {
		rx_buf = list_first_entry(&local_list, struct rkcif_rx_buffer,
					  list_free);
		if (rx_buf) {
...
static void sditf_get_hdr_mode(struct sditf_priv *priv)
{
	...
	if (cif_dev->terminal_sensor.sd) {
		ret = v4l2_subdev_call(cif_dev->terminal_sensor.sd, core, ioctl,
				       RKMODULE_GET_HDR_CFG, &hdr_cfg);
		if (!ret)
			     ...
			     struct v4l2_subdev_format *fmt)
{
	...
	if (cif_dev->terminal_sensor.sd) {
		...
		pixm.pixelformat =
			rkcif_mbus_pixelcode_to_v4l2(fmt->format.code);
		...
		if (priv->toisp_inf.link_mode == TOISP_UNITE &&
		    ((pixm.width / 2 - RKMOUDLE_UNITE_EXTEND_PIXEL) *
		     out_fmt->raw_bpp / 8) &
			    0xf)
			is_uncompact = true;
...
static int sditf_init_buf(struct sditf_priv *priv)
{
	...
	if (priv->hdr_cfg.hdr_mode == HDR_X2) {
		if (priv->mode.rdbk_mode == RKISP_VICAP_RDBK_AUTO) {
			...
			ret = rkcif_init_rx_buf(&cif_dev->stream[0],
						priv->buf_num);
			if (cif_dev->is_thunderboot)
				cif_dev->resmem_pa += cif_dev->resmem_size;
			ret |= rkcif_init_rx_buf(&cif_dev->stream[1],
						 priv->buf_num);
		} else {
			ret = rkcif_init_rx_buf(&cif_dev->stream[0],
						priv->buf_num);
		...
		if (priv->mode.rdbk_mode == RKISP_VICAP_RDBK_AUTO) {
			...
			if (cif_dev->is_thunderboot)
				cif_dev->resmem_pa += cif_dev->resmem_size;
			ret |= rkcif_init_rx_buf(&cif_dev->stream[2],
						 priv->buf_num);
		} else {
			ret = rkcif_init_rx_buf(&cif_dev->stream[0],
						priv->buf_num);
...
static void sditf_free_buf(struct sditf_priv *priv)
{
	...
	if (priv->hdr_cfg.hdr_mode == HDR_X2) {
		rkcif_free_rx_buf(&cif_dev->stream[0],
				  cif_dev->stream[0].rx_buf_num);
		rkcif_free_rx_buf(&cif_dev->stream[1],
				  cif_dev->stream[1].rx_buf_num);
	} else if (priv->hdr_cfg.hdr_mode == HDR_X3) {
		rkcif_free_rx_buf(&cif_dev->stream[0],
				  cif_dev->stream[0].rx_buf_num);
		rkcif_free_rx_buf(&cif_dev->stream[1],
				  cif_dev->stream[1].rx_buf_num);
		rkcif_free_rx_buf(&cif_dev->stream[2],
				  cif_dev->stream[2].rx_buf_num);
	} else {
		rkcif_free_rx_buf(&cif_dev->stream[0],
				  cif_dev->stream[0].rx_buf_num);
	}
...
static int sditf_channel_enable(struct sditf_priv *priv, int user)
{
	...
	struct rkmodule_capture_info *capture_info =
		&cif_dev->channels[0].capture_info;
	...
	if (priv->hdr_cfg.hdr_mode == NO_HDR ||
		...
		if (user == 0)
			int_en = CIF_TOISP0_FS(0) | CIF_TOISP0_FS(1) |
				 CIF_TOISP0_FS(2) | CIF_TOISP0_FE(0) |
				 CIF_TOISP0_FE(1) | CIF_TOISP0_FE(2);
		else
			int_en = CIF_TOISP1_FS(0) | CIF_TOISP1_FS(1) |
				 CIF_TOISP1_FS(2) | CIF_TOISP1_FE(0) |
				 CIF_TOISP1_FE(1) | CIF_TOISP1_FE(2);
		priv->toisp_inf.ch_info[0].is_valid = true;
		priv->toisp_inf.ch_info[0].id = ch0;
		priv->toisp_inf.ch_info[1].is_valid = true;
		priv->toisp_inf.ch_info[1].id = ch1;
...
void sditf_disable_immediately(struct sditf_priv *priv)
{
	...
	if (priv->toisp_inf.link_mode == TOISP0) {
		rkcif_write_register_and(cif_dev, CIF_REG_TOISP0_CTRL,
					 ~ctrl_val);
	} else if (priv->toisp_inf.link_mode == TOISP1) {
		rkcif_write_register_and(cif_dev, CIF_REG_TOISP1_CTRL,
					 ~ctrl_val);
	} else if (priv->toisp_inf.link_mode == TOISP_UNITE) {
		rkcif_write_register_and(cif_dev, CIF_REG_TOISP0_CTRL,
					 ~ctrl_val);
			     ...
			     unsigned int *size)
{
	...
	if (!list_empty(&stream->rx_buf_head) && cif_dev->is_thunderboot &&
	    ((!cif_dev->is_rtt_suspend && !cif_dev->is_aov_reserved) ||
	     !is_single_dev) &&
	...
	if (!is_free && (!dbufs->is_switch)) {
		...
		if (cif_dev->rdbk_debug) {
			...
			if (cif_dev->is_thunderboot ||
				...
				cif_dev->hw_dev->mem_ops->prepare(
	...
	if (!cif_dev->is_thunderboot || cif_dev->is_rdbk_to_online == false)
		return 0;
	...
	if (dbufs->runtime_us && cif_dev->early_line == 0) {
		...
		if (cif_dev->is_thunderboot)
			...
			diff_time = 1000000;
		if (dbufs->runtime_us * 1000 + cif_dev->sensor_linetime >
		    diff_time)
			early_time = dbufs->runtime_us * 1000 - diff_time;
		else
			early_time = diff_time;
		cif_dev->early_line =
			div_u64(early_time, cif_dev->sensor_linetime);
		cif_dev->wait_line_cache =
			sensor->raw_rect.height - cif_dev->early_line;
		if (cif_dev->rdbk_debug && dbufs->sequence < 15)
			v4l2_info(
				&cif_dev->v4l2_dev,
				"%s, isp runtime %d, line time %d, early_line %d, line_intr_cnt %d, seq %d, type %d, dma_addr %x\n",
				__func__, dbufs->runtime_us,
				cif_dev->sensor_linetime, cif_dev->early_line,
				cif_dev->wait_line_cache, dbufs->sequence,
	...
	case V4L2_CID_PIXEL_RATE:
		if (priv->cif_dev->terminal_sensor.sd) {
			sensor_ctrl = v4l2_ctrl_find(
				priv->cif_dev->terminal_sensor.sd->ctrl_handler,
				V4L2_CID_PIXEL_RATE);
			if (sensor_ctrl) {
				...
				v4l2_dbg(
					1, rkcif_debug,
					&priv->cif_dev->v4l2_dev,
					"%s, %s pixel rate %d\n", __func__,
					priv->cif_dev->terminal_sensor.sd->name,
					ctrl->val);
				return 0;
			} else {
				return -EINVAL;
...
static int rkcif_subdev_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(node, "rockchip,combine-index",
				   &priv->combine_index);
	if (ret) {

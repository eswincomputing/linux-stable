...
static int csi_composite_probe(struct platform_device *pdev)
{
	...
	v4l2_async_nf_init(&csi_dma->notifier);


    /*get the first fwnode*/
	ret = v4l2_async_nf_parse_fwnode_endpoints(
		csi_dma->dev, &csi_dma->notifier,
		sizeof(struct v4l2_async_subdev), csi_dma_fwnode_parse);
	if (ret)
		goto clean_out;

    /* get every fwnode on device tree with "remote-endpoint" field,
     * and and it to notifier list
     */
	list_for_each_entry (asd, &csi_dma->notifier.asd_list, asd_list) {
		if (asd->match.fwnode->dev == dev)
			continue;
		ret = v4l2_async_nf_parse_fwnode_endpoints(
			asd->match.fwnode->dev, &csi_dma->notifier,
			sizeof(struct v4l2_async_subdev), csi_dma_fwnode_parse);
		if (ret)
			goto clean_out;
	}

	csi_dma->notifier.ops = &csi_dma_graph_notify_ops;
	ret = v4l2_async_nf_register(&csi_dma->v4l2_dev, &csi_dma->notifier);

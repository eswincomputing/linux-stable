			  ...
			  struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		struct v4l2_mbus_framefmt *try_fmt = v4l2_subdev_get_try_format(
			&ov2718->subdev, sd_state, fmt->pad);
		/* update the code which could change due to vflip or hflip: */
		try_fmt->code = mode->bus_fmt;
		fmt->format = *try_fmt;
	} else {
		fmt->format.width = mode->width;
		fmt->format.height = mode->height;
...
static int ov2718_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		ov2718_read_reg(client, OV2718_REG_CTRL_MODE,OV2718_REG_VALUE_08BIT, &regval);
	} else {
...
static int __ov2718_power_on(struct ov2718 *ov2718)
{
	...
	if (clk_get_rate(ov2718->xvclk) != OV2718_XVCLK_FREQ)
		dev_dbg(dev, "xvclk mismatched, modes are based on 24MHz\n");
				  ...
				  struct i2c_client *client)
{
	...
	dev_info(dev, "Detected CHIP_ID: OV%06x sensor\n", CHIP_ID);
...
static int ov2718_analyze_dts(struct ov2718 *ov2718)
{
	...
	if (IS_ERR(ov2718->rst_gpio))
		dev_dbg(dev, "can not find rst-gpios, error %ld\n",
			 PTR_ERR(ov2718->rst_gpio));

	ov2718->pwd_gpio = devm_gpiod_get(dev, "pwdn", GPIOD_OUT_LOW);
	if (IS_ERR(ov2718->pwd_gpio))
		dev_dbg(dev, "can not find pwdn-gpios, error %ld\n",
...
static int ov2718_probe(struct i2c_client *client)
{
	...
	snprintf(sd->name, sizeof(sd->name), "m%s %s", OV2718_NAME, dev_name(sd->dev));

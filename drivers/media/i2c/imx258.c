...
static int imx258_start_streaming(struct imx258 *imx258)
{
	...
	dev_info(&client->dev, "%s: link_freq_index=%d, lane_mode_idx=%d\n",
			__func__, link_freq_index, imx258->lane_mode_idx);
...
static const struct v4l2_subdev_pad_ops imx258_pad_ops = {
	...
	.get_mbus_config = imx258_get_mbus_config,
};
...
static int imx258_probe(struct i2c_client *client)
{
	...
	if (val > 23520000 && val < 24480000) {
		/* If clock is in range 24MHz, use 24MHz link frequency */
		val = 24000000;
	} else if (val > 19000000 && val < 20000000) {
		/* If clock is in range 19.2MHz, use 19.2MHz link frequency */
		val = 19200000;
	} else {
	...
	case 2:
		...
		break;
	case 4:
		dev_info(&client->dev, "Using 4 data lanes\n");
	...
	dev_info(&client->dev, "IMX258 sensor probed successfully\n");

...
#define CTRL_OFST 0x10000
#define T_ZQ_OFST 0x100A4
...
#define STAT_UECNT_MASK 0xFFFF0000
#define STAT_CECNT_MASK 0xFFFF
#define STAT_UECNT_SHIFT 16
...
#define ECC_CFG0_OFST 0x10600
#define ECC_CFG1_OFST 0x10604

/* ECC Status Register */
#define ECC_STAT_OFST 0x10608

/* ECC Clear Register */
#define ECC_CLR_OFST 0x1060C

/* ECC Error count Register */
#define ECC_ERRCNT_OFST 0x10610

/* ECC Corrected Error Address Register */
#define ECC_CEADDR0_OFST 0x10614
#define ECC_CEADDR1_OFST 0x10618

/* ECC Syndrome Registers */
#define ECC_CSYND0_OFST 0x1061C
#define ECC_CSYND1_OFST 0x10620
#define ECC_CSYND2_OFST 0x10624

/* ECC Bit Mask0 Address Register */
#define ECC_BITMASK0_OFST 0x10628
#define ECC_BITMASK1_OFST 0x1062C
#define ECC_BITMASK2_OFST 0x10630

/* ECC UnCorrected Error Address Register */
#define ECC_UEADDR0_OFST 0x10634
#define ECC_UEADDR1_OFST 0x10638

/* ECC Syndrome Registers */
#define ECC_UESYND0_OFST 0x1063C
#define ECC_UESYND1_OFST 0x10640
#define ECC_UESYND2_OFST 0x10644

/* ECC Poison Address Reg */
#define ECC_POISON0_OFST 0x10648
...
#define ECC_CEADDR1_CID_MASK 0xF0000000
#define ECC_CEADDR1_COL_MASK 0x7FF
#define ECC_CEADDR1_CID_SHIFT 28
 ...
 */
static int eswin_get_error_info(struct synps_edac_priv *priv)
{
...
/**
 * eswin_get_dtype - Return the controller memory width.
 ...
 */
static enum dev_type eswin_get_dtype(const void __iomem *base)
{
	enum dev_type dt;
	u32 width;
...
/**
 * eswin_get_ecc_state - Return the controller ECC enable/disable status.
 * @base:	DDR memory controller base address.
 *
 ...
 */
static bool eswin_get_ecc_state(void __iomem *base)
{
	enum dev_type dt;
	u32 ecctype;

	dt = eswin_get_dtype(base);
	if (dt == DEV_UNKNOWN)
		return false;
...
/**
 * eswin_get_mtype - Returns controller memory type.
 ...
 */
static enum mem_type eswin_get_mtype(const void __iomem *base)
{
	enum mem_type mt;
	u32 memtype;
...
static const struct synps_platform_data eswin_edac_def = {
	.get_error_info = eswin_get_error_info,
	.get_mtype = eswin_get_mtype,
	.get_dtype = eswin_get_dtype,
	.get_ecc_state = eswin_get_ecc_state,
...
static void ddr_poison_setup(struct synps_edac_priv *priv)
{
	...
	for (index = 0; index < DDR_MAX_BANKGRP_SHIFT; index++)
	{
		if (priv->bankgrp_shift[index])
			bankgrp |= (((hif_addr >> priv->bankgrp_shift[index]) & BIT(0)) << index);
		else
			break;
	}
...
static void setup_bank_address_map(struct synps_edac_priv *priv, u32 *addrmap)
{
	...
	priv->bank_shift[2] = (((addrmap[1] >> 16) &
							BANK_MAX_VAL_MASK) == BANK_MAX_VAL_MASK)
							  ? 0
...
static void setup_bg_address_map(struct synps_edac_priv *priv, u32 *addrmap)
{
	...
	priv->bankgrp_shift[1] = (((addrmap[8] >> 8) & BANKGRP_MAX_VAL_MASK) ==
							  BANKGRP_MAX_VAL_MASK)
								 ? 0
								 : (((addrmap[8] >> 8) & BANKGRP_MAX_VAL_MASK) + BANKGRP_B1_BASE);
}

static void setup_rank_address_map(struct synps_edac_priv *priv, u32 *addrmap)
{
	priv->rank_shift[0] = ((addrmap[1] & RANK_MAX_VAL_MASK) ==
						   RANK_MAX_VAL_MASK)
							  ? 0
							  : ((addrmap[0] &
								  RANK_MAX_VAL_MASK) +
